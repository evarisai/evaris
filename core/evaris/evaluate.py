...
def _normalize_data(data: dict[str, Any] | Golden | TestCase) -> TestCase:
...
def _resolve_metrics(metrics_input: list[str | Any]) -> list[Any]:
...
) -> AgentFunction | AsyncAgentFunction:
    ...
    if is_sync_agent(task) or is_async_agent(task):
        ...
        if is_async_agent(task):
            ...
            async def async_wrapper(input: MultiModalInput) -> MultiModalOutput:
                result = await task.a_execute(input)  # type: ignore[union-attr]
            ...
            def sync_wrapper(input: MultiModalInput) -> MultiModalOutput:
                result = task.execute(input)  # type: ignore[union-attr]
                return cast(MultiModalOutput, result)
...
async def evaluate_stream(
    name: str,
    task: AgentFunction | AsyncAgentFunction,
    data: DatasetInput,
    metrics: list[str | Any],
    max_concurrency: int = 10,
    compliance_config: Any | None = None,
