...
class OracleValidator:
    ...
    def __init__(
        ...
        oracle_solver: Callable[[TestCase], Any] | None = None,
    ):
...
class OracleValidationResult(BaseModel):
    ...
    solvable: bool | None = Field(default=None, description="Whether oracle could solve task")
    oracle_output: Any | None = Field(default=None, description="Oracle's output for the task")
...
class BenchmarkValidationResult(BaseModel):
    ...
    solvable_tasks: int | None = Field(default=None, description="Number of tasks solved by oracle")
    unsolvable_tasks: int | None = Field(
